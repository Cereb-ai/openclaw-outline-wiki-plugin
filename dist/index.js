...
const configSchema = Type.Object({
    ...
    endpoint: Type.Optional(Type.String({
        description: "Outline API base URL, e.g. https://wiki.dev.cereb.ai/api",
    })),
    mcpEndpoint: Type.Optional(Type.String({
        description: "Outline MCP endpoint for S3 pre-signed attachment uploads, e.g. https://wiki.dev.cereb.ai/mcp",
    })),
...
export default defineToolPlugin({
    ...
    tools: (tool) => [
        ...
        tool({
            ...
            description: "Update an existing document's `text` and/or `title` (`editMode=replace` default). Required: `id` (UUID) + at least one of `text` or `title`. **Does NOT accept `parentDocumentId`** — the outline server silently drops it (verified 2026-06-08). To reparent, use `outline_doc_move`. Optional: `publish` (bool).",
            parameters: Type.Object({
                id: Type.String({ description: "Outline document UUID." }),
...
async function docUpdate(args, cfg) {
    ...
    if (typeof args.parentDocumentId === "string" && args.parentDocumentId.length > 0) {
        return textResult({
            ...
            hint: "Verified 2026-06-08 against wiki.dev.cereb.ai: documents.update ignores parentDocumentId, " +
...
function requireConfig(cfg) {
    if (!cfg.apiToken || !cfg.endpoint) {
        return textResult({
            error: "Outline Wiki plugin is not configured. Set `apiToken` (Bearer) and `endpoint` (e.g. https://wiki.dev.cereb.ai/api) under `plugins.entries.outline-wiki-openclaw-plugin.config` in openclaw.json.",
        });
    }
    return null;
}

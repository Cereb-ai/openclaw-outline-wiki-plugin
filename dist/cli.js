...
async function dispatchDoc(method, args, cfg) {
    switch (method) {
        ...
        case "create": {
            ...
            if (!collectionId) {
                return textResult({
                    error: "doc.create requires `collectionId` (string) — pass it as an arg, or set `defaultCollectionId` in the plugin config.",
                });
            }
            const body = {
                title: args.title,
        ...
        case "update": {
            ...
            if (args.parentDocumentId !== undefined) {
                return textResult({
                    error: "doc.update does not accept `parentDocumentId` — the outline server silently drops it. " +
...
async function dispatchAttachment(method, args, cfg) {
    ...
    if (typeof args.url !== "string" && typeof args.path !== "string") {
        return textResult({
            error: "attachment.upload requires either `url` (string, outline fetches it) or `path` (string, plugin reads + uploads).",
        });
    }
    ...
    if (typeof args.url === "string") {
        if (preset !== "documentAttachment") {
            return textResult({
                error: "attachment.upload with `url` only supports preset=documentAttachment; use `path` for avatar/emoji.",
            });
        }
        if (typeof args.documentId !== "string" || args.documentId.length === 0) {
            return textResult({
                error: "attachment.upload with `url` requires `documentId` (string, UUID) — outline's createFromUrl endpoint refuses document attachments without a target document.",
            });
        }
